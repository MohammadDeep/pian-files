...
dir_save_modeles = '/home/asr/mohammadBalaghi/pian-files/__HISTOY_MODEL/haag_ecg_signall_p_85'
...
SELECTED_CH = [7]   # ← این را مطابق دادهٔ خودت عوض کن
IN_CH = len(SELECTED_CH)     # مهم: مدل‌ها با این مقدار sync شوند
'''
====================================================================
                            Create dataset
====================================================================                            
...
class ShardedNPYDataset(Dataset):
    def __init__(self, root_dir,
                 ...
                 dtype_y=torch.long,
                 channel_idx=None,   # NEW: لیست اندیس کانال‌ها؛ مثل [0,2,5]
                 ):
        ...
        self.channel_idx = None if channel_idx is None else np.asarray(channel_idx, dtype=np.int64)
        ...
        for x_path, y_path in zip(xs, ys):
            ...
            # اگر channel_idx داده شده، در همین‌جا صحت‌سنجی کن
            if self.channel_idx is not None:
                C = X.shape[1]
                assert np.all(self.channel_idx >= 0) and np.all(self.channel_idx < C), \
                    f"channel_idx خارج از بازهٔ ۰..{C-1}"
        ...
        if self.normalize is not None:
            ...
            # شکل‌دهی برای broadcast: (C_sel, 1)
            self.norm_mean = torch.from_numpy(m)[:, None]
            self.norm_std  = torch.from_numpy(s)[:, None]
        else:
    ...
    def __getitem__(self, idx):
        ...
        x_np = X[k]                # (C, T)
        if self.channel_idx is not None:
            x_np = x_np[self.channel_idx]   # (C_sel, T) — بدون کپی (mmap view)

        y_np = y[k]                # ()

        x = torch.from_numpy(np.array(x_np, copy=False)).to(self.dtype_x)  # (C_sel,T)
...
def compute_channel_stats(root_dir, x_pattern="X_*.npy", eps=1e-12, channel_idx=None):
    ...
    if channel_idx is None:
        idx = np.arange(C_full, dtype=np.int64)
    else:
        idx = np.asarray(channel_idx, dtype=np.int64)
        assert np.all(idx >= 0) and np.all(idx < C_full), "channel_idx نامعتبر است"
    ...
    for x_path in xs:
        X = np.load(x_path, mmap_mode='r')      # (N, C_full, T)
        Xs = X[:, idx, :]                       # (N, C, T) — view
        N, _, T = Xs.shape

        # میانگین و میانگین مربع روی (N,T)
        m = Xs.mean(axis=(0,2))                 # (C,)
        v = (Xs**2).mean(axis=(0,2))            # (C,)
